...
|----|----------|-------------|------|----------|
| TC-A1 | Вход существующего клиента | BE + DB + seed; Web на `localhost:8081` | 1. Телефон `+79990000001` → «Получить код» 2. Ввести OTP с экрана / из ответа API 3. Подтвердить | Переход в приложение, список слотов доступен |
...
| TC-S2 | Фильтр по типу трассы | Список слотов загружен | 1. Открыть фильтры (BS-001) 2. В группе «Уровень» выбрать «новичковая» | Только слоты novice-маршрутов |
| TC-S2a | Фильтр по имени маршрута | Список слотов загружен | 1. Фильтры (BS-001) 2. В группе «Маршрут» выбрать «Короткая трасса» | Слоты только этой конфигурации; AND с «Уровень» |
...
| 7 | TC-S2a | UI | Фильтр «Маршрут» | ✅ Группы «Уровень» + «Маршрут» в BS-001 (коммит `8438d96`) |
| 8 | TC-S3 | UI | Карточка слота | ✅ Маршалы и адреса на кириллице после BUG-002/003 fix |
| 9 | TC-S4 | API | createBooking + price | ✅ `TestCreateBookingFlowAndIdempotency` (5800 ₽) |
| 10 | TC-S5 | UI/seed | Полный слот | ✅ пн 15:00 `free_seats=0` в demonstration seed |
| 11 | TC-B1 | UI | Список броней | ✅ `TestListAndGetBookings` + UI у Ивана |
| 12 | TC-B2 | UI | Детали брони | ✅ `BookingDetailsStoreTest` (wasm) |
| 13 | TC-B3 | API | Early cancel | ✅ `TestCancelBookingEarlyLateAndAfterStart` |
| 14 | TC-B4 | API | Late cancel | ✅ тот же тест, ветка `< 1h` |
| 15 | — | Auto | Idempotency / concurrency | ✅ `TestCreateBookingConcurrencyDoesNotOverbook` |
| 16 | — | Auto | Domain policies (client) | ✅ `DomainPolicyTest` (wasmJsTest) |
...
| Метрика | Значение |
|---------|----------|
| Ручных тест-кейсов | 14 (TC-A1…TC-B4, TC-S2a) |
