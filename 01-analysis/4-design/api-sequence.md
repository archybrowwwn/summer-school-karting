...
> - Все вызовы — с `Authorization: Bearer <token>` (`bearerAuth`); при истёкшем/неверном токене сервер отвечает `401`, клиент уходит на вход [SCR-001](../3-design-brief/SCR-001-registration.md).
> - Сервер — **источник истины** по времени и доступности: `slot.start_at` в UTC, тип отмены и наличие мест/комплектов экипировки проверяет сервер, клиент их не пересчитывает (R-005, R-021).
...
sequenceDiagram
    ...
    Note over App: SCR-004: выбраны места/комплекты экипировки,<br/>цена показана из price_total слота
    User->>App: Тап «Записаться на заезд»
    ...
    else Нет свободных мест/комплектов экипировки или двойная бронь (409 Conflict)
        API-->>App: 409 {code: slot_full / double_booking,<br/>available_seats, available_rental_gear}
        App-->>User: Сообщение о нехватке мест/комплектов экипировки,<br/>обновление доступности слота
...
| Проверка | Сервер атомарно проверяет места/комплекты экипировки и фиксирует цену слота | NFR-8/9, R-010 |
| `201` | Возвращается `Booking` со `status=active` и `price_total` (read-only) | R-005 |
| `409` | Нет мест/комплектов экипировки или двойная бронь; тело несёт `available_seats`/`available_rental_gear` (Error.details) | common/models.yaml |
...
sequenceDiagram
    ...
    alt Ранняя отмена (≥ 1 ч)
        ...
        App-->>User: Отмена недоступна после старта заезда
...
> Полная модель состояний брони и инварианты освобождения мест/комплектов экипировки —
