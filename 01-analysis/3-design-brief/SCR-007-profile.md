...
> Документ описывает **функционально-структурные** требования к экрану «Профиль клиента» для приложения картинг-центра.
...
[Бриф от заказчика (Денис)](../1-elicitation/brief-karting.md)
...
- **Настроить предпочтения по экипировке** — выбор по умолчанию, использует ли клиент свой шлем и подшлемник или берёт напрокат (ускоряет флоу записи на заезд).
...
- **Справочные пункты** — правила картодрома, контакты поддержки, версия приложения.

Это не «личный кабинет» в широком смысле: экран не содержит истории заездов (для этого есть
...
| Параметр | Значение |
|----------|----------|
| **Роль** | Клиент (единственная роль мобильного приложения). |
| **Контекст** | Чаще всего — на открытом уличном картодроме: шум моторов, яркое солнце, руки в гоночных перчатках или уставшие после заезда (см. [foundations §1](00-foundations.md#1-продукт-и-аудитория)). Заход в «Профиль» обычно эпизодический: свериться с данными, переключить дефолтную экипировку или выйти на чужом/общем устройстве. |
| **Задача (Job)** | «Хочу убедиться, что вошёл под своим именем», «хочу указать, что у меня свой шлем, чтобы не выбирать это каждый раз при записи» и «хочу безопасно выйти, чтобы под моим аккаунтом не записались другие». |
| **Цель дизайна** | Считываемость данных с одного взгляда на солнце; крупные элементы под управление в перчатках; защита от случайного выхода. |
...
| `has_own_equipment` | «Своя экипировка» (шлем, подшлемник) | Клиент | read + **edit** (свитчер в режиме просмотра) |
...
| Справочные пункты | «Правила картодрома», «Поддержка», «Версия приложения» | конфиг / статика | read; правила/поддержка — переходы, версия — текст |
...
3. **Настройки** — свитчер «Своя экипировка».
4. **Справочные пункты** — «Правила картодрома» ›, «Поддержка» ›, «Версия приложения».
5. **Кнопка «Выйти»** — визуально отделена, в безопасной зоне (см. §9).
6. **Кнопка «Удалить аккаунт»** — деструктивная, ниже «Выйти», менее акцентная, с подтверждением.
7. **Таб-бар** — постоянный (активна вкладка «Профиль»).
...
├─────────────────────────────┤
│  Имя                         │
│  Алексей Смирнов             │  ← блок данных
│  Телефон                     │
│  +7 999 123-45-67            │
│                              │
│  Своя экипировка         (ВКЛ)  ← свитчер
│  (шлем, подшлемник)          │
│  · · · · · · · · · · · · ·   │
│  Правила картодрома       ›  │  ← справочные пункты
│  Поддержка                ›  │
...
### 6.2 Свитчер «Своя экипировка»
- Переключатель (Toggle), позволяющий задать предпочтение по умолчанию. Состояние сохраняется на бэкенде сразу при переключении (оптимистичный UI с rollback при ошибке сети).
- Значение используется как предзаполненное поле при бронировании нового заезда (ускоряет запись).

### 6.3 Кнопка «Выйти»
- Завершает сессию и переводит на [SCR-001](SCR-001-registration.md).
- **Обязательное подтверждение** перед выходом — защита от случайного нажатия в суете на картодроме (P1). Подтверждение оформляется как диалог/шторка по правилам
...
### 6.5 Справочные пункты
- **Правила картодрома** и **Поддержка** — строки-переходы (›) на статические/внешние ресурсы (адрес/ссылка — параметр конфигурации).
...
|-----------|------------------------|
| **Loading** | Скелетон на месте блока данных (две строки «лейбл/значение») и блока настроек. Кнопка «Выйти» может быть видна, но неактивна до загрузки. |
| **Content (просмотр)** | Имя и телефон, тумблер экипировки, справочные пункты, кнопки «Редактировать», «Выйти», «Удалить аккаунт». |
...
- **Доступ только к собственным данным** (NFR-11, NFR-12): на клиенте не запрашиваются и не
  отображаются данные других пользователей; UI не содержит элементов администрирования (интерфейсы маршалов и владельца вынесены за скоуп приложения).
...
| Пояснение удаления | «Ваши данные и будущие записи на заезды будут удалены безвозвратно. Это действие нельзя отменить.» |
...
- **Крупная, читаемая контактная информация** — имя и телефон контрастны и комфортны для
  чтения на улице при ярком солнце (NFR-1, [foundations §3.2](00-foundations.md#32-контраст-и-читаемость-nfr-1)).
...
- **Кнопка «Выйти»** — тач-зона ≥ 44–48 pt, в **явной безопасной зоне**: отделена отступом от
  блока данных и не примыкает вплотную к таб-бару, чтобы исключить случайный выход (особенно при управлении в перчатках).
  Подтверждение (§6.3) — дополнительный барьер.
- Все элементы имеют текстовые подписи/доступные имена; статус выхода и переключения экипировки озвучивается screen reader’ом.
...
Feature: SCR-007 Профиль клиента
  ...
  Scenario: AC-001 — отображение собственных данных
    ...
    Тогда он видит своё имя, свой телефон и статус экипировки
  ...
  Scenario: AC-004 — нет доступа к чужим и административным данным
    ...
    И отсутствуют любые функции маршала или администратора
  ...
  Scenario: AC-009 — переключение экипировки по умолчанию
    Дано клиент находится на экране «Профиль» в режиме просмотра
    Когда он переключает тумблер «Своя экипировка»
    Тогда новое состояние сохраняется в профиле
    И при следующей записи на заезд это значение используется по умолчанию
```
...
2. **Своя экипировка как глобальная настройка — да.** Денис упоминал важность понимания, нужна ли клиенту экипировка. Вынос этого в профиль как настройки по умолчанию упрощает флоу бронирования слота (клиенту не нужно выбирать это каждый раз).
3. **Удаление аккаунта — да.** Кнопка «Удалить аккаунт» с явным подтверждением и пояснением про ПДн; ниже «Выйти» (§6.4).
4. **Справочные пункты — да.** Правила картодрома, поддержка, версия приложения (§6.5); адреса/ссылки — параметр конфигурации.
5. **Управление уведомлениями — НЕ в профиле.** Push-разрешение запрашивается в [BS-002](BS-002-booking-success.md); отдельного раздела настроек уведомлений в MVP нет ([foundations §8.1](00-foundations.md#81-напоминания-и-уведомления-fr-33-nfr-13)).
...
| Связь | Источник |
|-------|----------|
| **Бриф (Денис)** | «Хорошо бы чтобы выбирал, со своей экипировкой он... или дать напрокат» → реализовано через дефолтную настройку профиля. «Пользоваться будут в основном с телефона» → mobile-first. |
| **FR-1, FR-2** | Регистрация/авторизация → данные профиля (имя, телефон), выход и возврат к входу. |
| **NFR-11, NFR-12** | Доступ только к собственным данным; отсутствие чужих/административных сведений в UI (роль Клиент). |
