...
в уличном картинг-центре. Заменяет ручную запись через Telegram и маркерную доску.
...
> - **Свободные карты** (за раз можно выбрать): `max_karts = min(free_karts, config.capacity_cap)`. У новичковых трасс лимит может быть строже.
