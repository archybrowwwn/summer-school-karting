...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Карта маршрута (71:6176)](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6176)

> **Адаптация для картинг-центра:** вместо интерактивной карты маршрута — схема трассы + информация о месте сбора и локации центра. Кнопка **«Открыть в Яндекс.Картах»** ведёт к адресу центра. Текст «Заезд по конфигурации займёт N минут» (N = `route.duration_min`). Кнопка «Проложить маршрут» — deep-link навигации до места сбора у трассы.
...
|----------|---------|---------|------------------------|
| [SCR-003 · Карточка заезда](SCR-003-slot-card.md) | Тап по статичному превью карты/схемы трассы (CTA «Открыть схему ›») | Всегда | `route.name`, `route.type`, `route.duration_min`, `slot.meeting_point`, `slot.meeting_point_lat`, `slot.meeting_point_lng`, `center_address` |
| [SCR-006 · Детали брони](SCR-006-booking-details.md) | Тап по статичному превью карты/схемы трассы (CTA «Открыть схему ›») | Всегда | `route.name`, `route.type`, `route.duration_min`, `slot.meeting_point`, `slot.meeting_point_lat`, `slot.meeting_point_lng`, `center_address` |
...
>
> **Правило форматирования строки конфигурации** (текстовый блок под схемой): `route.name · {Короткая|Длинная} · ~{route.duration_min} мин`, например «Короткая трасса · Короткая · ~15 мин». Где `label(route.type)`: `novice` → «Короткая», `experienced` → «Длинная». Сегмент «~N мин» **скрывается целиком**, если `duration_min` отсутствует; разделитель ` · ` ставится только между присутствующими сегментами.

| Название | Тип | Возможные значения | Описание |
|----------|-----|-------------------|----------|
| `route.name` | Состояние | строка, напр. `Короткая трасса` | Название конфигурации; дублируется текстом под схемой. |
| `route.type` | Состояние | `novice`, `experienced` | Тип конфигурации (короткая / длинная); дублируется текстом под схемой. |
| `route.duration_min` | Состояние | целое число минут, напр. `15`; может отсутствовать | Длительность заезда; в текстовом блоке показывается, если значение есть. |
...
|--------|-----------------|----------|
| [LOGIC-006 Схема трассы](09_Логики/LOGIC-006_Track_Map_and_Position.md) | Интерактивная схема / карта (FB-MAP-002), кнопки handoff, фолбэк | Отрисовка карты локации центра или статичной схемы трассы средствами Yandex Maps JS API; handoff во внешнее приложение; фолбэк на текст при недоступности; ключ/стиль из Remote Config. |
...
> К нашему REST/GraphQL API шторка запросов не делает. Данные о слоте/брони получены ранее через [`../api/slots/api.yaml`](../api/slots/api.yaml) → `getSlot` (для SCR-013) / [`../api/bookings/api.yaml`](../api/bookings/api.yaml) → `getBooking` (для SCR-006) и переданы в BS-004 как [входные данные](#входные-данные). Ниже — описание интеграции с внешним сервисом Яндекс.Карты.

### Интеграция: Yandex Maps JS API (схема локации / трассы)

**Тип:** Внешний сервис (клиентский SDK / JS API)  
**Спецификация:** Yandex Maps JS API. Логика: [LOGIC-006 Схема трассы](09_Логики/LOGIC-006_Track_Map_and_Position.md)  
...
│  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ [＋] [－] │  ← контролы зума
├─────────────────────────────────────┤
│  Короткая трасса · Короткая · ~15 мин │  ← route.name · type · duration_min
│  Заезд по конфигурации займёт 15 минут│  ← текст из route.duration_min
│  📍 Место сбора                       │
...
**Логика:**
- Карта/схема: [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md) — построение через Yandex Maps JS API или статичная схема, авто-fit на пин, фолбэк на текст при недоступности; смысл не передаётся только цветом (контраст не ниже WCAG AA).
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Название и тип конфигурации | Что за заезд (короткая / длинная) | `route.name`, `route.type` | — | — |
...
**Логика:**
- Блок: [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md) — текстовый эквивалент схемы, обязателен для доступности (NFR-1); виден во всех состояниях (загрузка / Content / фолбэк).
...
**Логика:**
- Кнопки: [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md) — handoff во внешнее картографическое приложение (deeplink); turn-by-turn выполняется внешним приложением, in-app навигации в MVP нет; при невозможности открытия — системная обработка / снек, клиент остаётся в приложении.
...
| ID | Название | Приоритет |
|----|----------|-----------|
| INT-YANDEX-MAPS | Интеграция с Яндекс.Картами: Maps JS API (схема локации BS-004), handoff во внешнее приложение; ключ и стиль тайлов — Remote Config — [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md) | Medium |
...
| AC-003a | **Дано** переданы `route.name`, `route.type` и `route.duration_min`, **Когда** строится текстовый блок, **Тогда** строка конфигурации форматируется как `route.name · {Короткая|Длинная} · ~{duration_min} мин`; при отсутствии `duration_min` сегмент «~N мин» и его разделитель скрываются (см. [Входные данные](#входные-данные)). | P1 |
