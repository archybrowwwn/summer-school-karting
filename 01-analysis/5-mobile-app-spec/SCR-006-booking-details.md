...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Детали записи (71:6100)](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6100)
...
| 0.2.0 | SCR-006 «Детали брони + отмена» | Адаптировано под картинг-центр «Апекс» на основе brief-karting.md и clarification (R-004, R-008, R-015, R-027, R-028): изменена терминология (заезд/конфигурация трассы/маршал/экипировка вместо заезд/маршрут/инструктор/экипировка), обновлены описания блоков, User Story, бизнес-ценность, критерии приёмки под картинг (включая club_cancelled по погоде/организатору, правило 1 часа для ранней/поздней отмены). Стиль, структура, все таблицы, mermaid, AC полностью сохранены. |
...
Две задачи экрана:
1. **Просмотр.** Собрать в одном месте все параметры брони и её текущий статус: на что и когда записан клиент, конфигурацию трассы и её тип, маршала, сколько мест и какая экипировка (прокатная/своя), итоговую цену (оплата офлайн) и служебные даты.
2. **Отмена.** Дать единственное действие над записью — отменить участие. Подтверждение и пояснение последствий (правило 1 часа) выполняется в шторке [BS-003](BS-003-cancel-confirm.md); экран лишь запускает её и после возврата отображает обновлённый статус.
...
> Как клиент картинг-центра «Апекс», я хочу видеть детали своей брони на заезд и при необходимости отменить её до старта,
...
- Ранняя отмена возвращает места в заезд (FR-17) — их можно перезанять, выше заполняемость заездов.
...
| [BS-004 Схема трассы](BS-004-route-map.md) | Тап по схеме трассы / «Открыть схему» | `bookingId` |
...
| [LOGIC-004 Отмена: правило 1 часа](09_Логики/LOGIC-004_Отмена-правило-2-часов.md) | Кнопка «Отменить», подсказка дедлайна | Граница ранней/поздней отмены, момент отсечки `start_at − 1 ч`, доступность CTA до старта. |
| [LOGIC-006 Карта маршрута](09_Логики/LOGIC-006_Track_Map_and_Position.md) | Блок «Карта маршрута + место встречи» | Статичный превью Яндекс-карты с линией маршрута и пином; состояния загрузки/ошибки и текстовый fallback; переход в [BS-004](BS-004-route-map.md). |
...
> При открытии экрана отправляется один критичный запрос `getBooking`. Схема трассы рендерится из уже полученных полей вложенного `slot.route` (отдельного API-вызова не требует, см. [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md)).
...
**Ответ:** схема `Booking` (`../api/bookings/models.yaml` → `Booking`) с вложенным `slot` (`../api/slots/models.yaml` → `Slot`), внутри которого `route` (конфигурация трассы) и `instructor` (маршал, `../api/instructors/models.yaml` → `Instructor`).
...
│                               │
│  Конфигурация трассы          │
│   Короткая (новичок)          │  ← slot.route.name + route.type
│   Маршал: Анна                │  ← slot.instructor.name
│                               │
│  ░░ схема трассы (превью) ░░░ │  ← статичный превью схемы трассы (LOGIC-006)
│  ░╱‾‾╲__ линия трассы   ░ 📍  │  ← slot.route.geometry + пин места сбора
│  [ Открыть схему › ]          │  ← тап → BS-004 (интерактивная схема)
│  📍 Место сбора               │
│   У трассы, бокс 3            │  ← slot.meeting_point (текст)
│                               │
│  Места и экипировка           │
│   Мест: 2                     │  ← seats_count
...
│  можно до 12 июля, 07:00      │     start_at − 1 ч (LOGIC-004)
...
| Блок «Конфигурация трассы и маршал» | Название и тип конфигурации трассы, имя маршала. | Да |
| Схема трассы + место сбора | Статичный превью схемы трассы + текст места сбора. | Да |
| Блок «Места и экипировка» | Число мест и разбивка прокатная/своя экипировка. | Да |
...
| Подсказка дедлайна | Момент бесплатной отсечки `start_at − 1 ч`. | Опционально (только при активной CTA) |
...
- Статус определяется по `status` и времени старта (см. [LOGIC-004](09_Логики/LOGIC-004_Отмена-правило-2-часов.md), [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md)):
  ...
  - **Поздняя отмена** — `status = late_cancel` (место не освобождено; текст-итог «Поздняя отмена: место не освобождено (правило 1 часа). Штраф не взимается.» — из [foundations §6](../3-design-brief/00-foundations.md#6-tone-of-voice-и-общая-микрокопия)).
  - **Отменена центром** — `status = club_cancelled` (бронь отменил центр/организатор или по погоде; места освобождены). Под бейджем показывается текст причины отмены из `cancellation_reason` (если заполнен). (R-008)
...
### 4. Блок «Конфигурация трассы и маршал»

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Название конфигурации | Имя конфигурации трассы | `slot.route.name` | — | — |
| Тип конфигурации | Короткая (для новичков) / Длинная (для опытных) | `slot.route.type` (`novice` / `experienced`) | — | — |
| Маршал | Имя ведущего | `slot.instructor.name` | — | — |

### 5. Схема трассы + место сбора

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Статичная схема | Превью схемы трассы с линией и пином места сбора | `slot.route.geometry` + `slot.meeting_point_lat`/`meeting_point_lng` | — | Открыть [BS-004](BS-004-route-map.md) |
| «Открыть схему ›» | Ссылка/кнопка перехода к интерактивной схеме | — | — | Открыть [BS-004](BS-004-route-map.md) |
| Место сбора (текст) | Адрес/ориентир под схемой (обязателен) | `slot.meeting_point` | — | — |

**Логика:**
- Схема трассы: [LOGIC-006](09_Логики/LOGIC-006_Track_Map_and_Position.md) — статичный превью схемы трассы; при загрузке — скелетон в форме карты; при ошибке/офлайн/отсутствии ключа — fallback на текстовый блок места сбора + ссылку «Открыть в Яндекс.Картах». Остальные детали брони при ошибке схемы остаются доступны.
- Текстовый блок места сбора обязателен и служит текстовым эквивалентом схемы (доступность).
...
- Для `status = club_cancelled` дополнительно показывается текст причины отмены из `cancellation_reason` (если заполнен) — рядом с бейджем «Отменена центром» / в блоке деталей отмены. (R-008)

### 9. Нижний CTA «Отменить» + подсказка дедлайна

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Подсказка дедлайна | «Бесплатно освободить место можно до `<start_at − 1 ч>`» | вычисляется из `slot.start_at` | — | — |
...
- Подсказка дедлайна показывается только при активной кнопке; момент отсечки `start_at − 1 ч` вычисляется из данных, без хардкода. Граница трактуется как **ранняя** отмена: точка `start_at − 1 ч` входит в «раннюю» (`≥ 1 ч` = ранняя → место освобождается), синхронно с [LOGIC-004](09_Логики/LOGIC-004_Отмена-правило-2-часов.md) и [foundations §6](../3-design-brief/00-foundations.md#6-tone-of-voice-и-общая-микрокопия). До самой отсечки включительно («бесплатно освободить место можно до `<start_at − 1 ч>`») — ранняя; позже — поздняя.
- Полный текст правила 1 часа на этом экране **не дублируется** — он раскрывается в [BS-003](BS-003-cancel-confirm.md); здесь только краткая подсказка/момент отсечки.
...
| 200 `status = late_cancel` (поздняя) | **Успех** (не ошибка) | Бейдж «Поздняя отмена» + текст-итог, показан `cancelled_at`; CTA disabled «Запись уже отменена» | «Поздняя отмена: место не освобождено (правило 1 часа). Штраф не взимается.» ([§6.1](../3-design-brief/00-foundations.md#61-каталог-снеков-успеха-единые-формулировки)) | SCR-006 (после закрытия шторки) |
...
**Условия доступности:**
- Кнопка «Отменить» **активна (enabled)**, только если: `status = active` **И** `slot.start_at` в будущем (заезд не началась).
...
| Content (Отменена центром) | 200 + `status = club_cancelled` | Полные детали; бейдж «Отменена центром» + текст причины `cancellation_reason`; показан `cancelled_at`; CTA disabled «Запись уже отменена» (UC-2 E2) |
...
| Открыть схему | Схема трассы / «Открыть схему ›» | Tap | Открыть [BS-004](BS-004-route-map.md) (`bookingId`) |
...
|----|----------|-----------|
| FR-16 | Отмена своей записи до старта заезды | Must |
| FR-17 | Ранняя отмена (≥ 1 ч) — места возвращаются в слот | Must |
| FR-18 | Поздняя отмена (< 1 ч) — статус «поздняя отмена» без освобождения места и без штрафов | Must |
...
| AC-002 | **Дано** успешный ответ `getBooking`, **Когда** экран в Content, **Тогда** показан статичный превью схемы трассы с линией и пином, а под схемой — текстовый блок места сбора (`slot.meeting_point`) | P0 |
...
| AC-004 | **Дано** активная кнопка «Отменить», **Когда** экран в Content, **Тогда** рядом показана подсказка с моментом отсечки «до `<start_at − 1 ч>`», вычисленным из данных без хардкода | P1 |
...
| AC-011 | **Дано** бронь со статусом `club_cancelled` (отменена центром), **Когда** экран в Content, **Тогда** показан бейдж «Отменена центром» и текст причины из `cancellation_reason` (если заполнен), а кнопка «Отменить» disabled с пояснением «Запись уже отменена» | P1 |
| AC-006 | **Дано** тап по схеме или «Открыть схему», **Когда** схема доступна, **Тогда** открывается [BS-004](BS-004-route-map.md) с интерактивной схемой трассы | P1 |
...
| AC-010 | **Дано** успешная **поздняя** отмена в [BS-003](BS-003-cancel-confirm.md) (200 `late_cancel` — **успешный** исход, не ошибка), **Когда** шторка закрылась, **Тогда** SCR-006 показывает снек «Поздняя отмена: место не освобождено (правило 1 часа). Штраф не взимается.» ([foundations §6.1](../3-design-brief/00-foundations.md#61-каталог-снеков-успеха-единые-формулировки)), бейдж «Поздняя отмена» и `cancelled_at` | P1 |
...
| AC-N02 | **Дано** `status = active`, но `slot.start_at` уже наступил (заезд началась/прошла, UC-2 E1), **Когда** экран в Content, **Тогда** кнопка «Отменить» disabled с пояснением «Заезд уже начался — отменить запись нельзя» | P0 |
...
| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** до старта осталось ровно 1 часа, **Когда** клиент видит подсказку и запускает отмену, **Тогда** отмена трактуется как ранняя (`≥ 1 ч` → место освобождается), подсказка показывает момент отсечки `start_at − 1 ч` | P1 |
| AC-E02 | **Дано** до старта осталось менее 1 часа, но заезд ещё не началась, **Когда** экран в Content, **Тогда** кнопка «Отменить» активна (доступна до старта), а отмена в [BS-003](BS-003-cancel-confirm.md) будет поздней | P1 |
...
| AC-E04 | **Дано** все места брони — свои (`rental_count = 0`), **Когда** экран в Content, **Тогда** в блоке «Места и экипировка» показано число мест и «своя экипировка» без прокатной, числа считаются из `seats_count`/`rental_count` | P2 |
