...
| 0.2.0 | SCR-006 «Детали брони + отмена» | Адаптировано под картинг-центр «Апекс» на основе brief-karting.md и clarification (R-004, R-008, R-015, R-027, R-028): изменена терминология (заезд/конфигурация трассы/маршал/экипировка вместо заезд/маршрут/инструктор/доска), обновлены описания блоков, User Story, бизнес-ценность, критерии приёмки под картинг (включая club_cancelled по погоде/организатору, правило 2 часов для ранней/поздней отмены). Стиль, структура, все таблицы, mermaid, AC полностью сохранены. |
...
> При открытии экрана отправляется один критичный запрос `getBooking`. Схема трассы рендерится из уже полученных полей вложенного `slot.track_config` (отдельного API-вызова не требует, см. [LOGIC-006](09_Логики/LOGIC-006_Карта-маршрута.md)).
...
**Ответ:** схема `Booking` (`../api/bookings/models.yaml` → `Booking`) с вложенным `slot` (`../api/slots/models.yaml` → `Slot`), внутри которого `track_config` (конфигурация трассы) и `marshal` (`../api/marshals/models.yaml`). (Адаптировано под картинг; backend предоставляет track_config.name, track_config.type, marshal.name, meeting_point и geometry для превью.)
...
│                               │
│  Конфигурация трассы          │
│   Короткая (новичок)          │  ← slot.track_config.name + track_config.type
│   Маршал: Анна                │  ← slot.marshal.name
│                               │
│  ░░ схема трассы (превью) ░░░ │  ← статичный превью схемы трассы (LOGIC-006)
│  ░╱‾‾╲__ линия трассы   ░ 📍  │  ← slot.track_config.geometry + пин места сбора
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Название конфигурации | Имя конфигурации трассы | `slot.track_config.name` | — | — |
| Тип конфигурации | Короткая (для новичков) / Длинная (для опытных) | `slot.track_config.type` (`short` / `long`) | — | — |
| Маршал | Имя ведущего | `slot.marshal.name` | — | — |

### 5. Схема трассы + место сбора

| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Статичная схема | Превью схемы трассы с линией и пином места сбора | `slot.track_config.geometry` + `slot.meeting_point_lat`/`meeting_point_lng` | — | Открыть [BS-004](BS-004-route-map.md) |
...
**Логика:**
- Схема трассы: [LOGIC-006](09_Логики/LOGIC-006_Карта-маршрута.md) — статичный превью схемы трассы; при загрузке — скелетон в форме карты; при ошибке/офлайн/отсутствии ключа — fallback на текстовый блок места сбора + ссылку «Открыть в Яндекс.Картах». Остальные детали брони при ошибке схемы остаются доступны.
...
| AC-N02 | **Дано** `status = active`, но `slot.start_at` уже наступил (заезд началась/прошла, UC-2 E1), **Когда** экран в Content, **Тогда** кнопка «Отменить» disabled с пояснением «Прогулка уже началась — отменить запись нельзя» | P0 |
...
| AC-E02 | **Дано** до старта осталось менее 2 часов, но заезд ещё не началась, **Когда** экран в Content, **Тогда** кнопка «Отменить» активна (доступна до старта), а отмена в [BS-003](BS-003-cancel-confirm.md) будет поздней | P1 |
