...
|-------|-----|-------------------|
| 0.1 | SCR-002 «Список заездов» | Первоначальная документация экрана. Адаптировано под картинг-центр «Апекс» на основе брифа заказчика (заезды вместо прогулок, конфигурация трассы, маршал, экипировка). |
...
| `appliedFilters.track_config` | Состояние | массив из `short_novice`, `long_experienced`; по умолчанию `[]` | Конфигурации трассы (OR внутри группы); пустой массив = любая конфигурация. |
...
| `track_config` | array(enum) `short_novice`/`long_experienced` | Нет | `appliedFilters.track_config` | Конфигурации трассы, OR внутри группы; опускается, если массив пуст. |
...
`price`, `rental_price_equipment`, `total_seats`, `free_seats`, `free_rental_equipment`, `status`.
...
│  │ Короткая трасса · Новичок       │ │  ← track.config (short_novice)
...
| Конфигурация трассы | Что за заезд (короткая/длинная, новичок/опытный) | `track.config` из `listSlots` | — | — |
| Маршал | Кто ведёт | `marshal.name` из `listSlots` | — | — |
...
**Логика:**
- Список: [LOGIC-005](09_Логики/LOGIC-005_Фильтрация-слотов.md) — сортировка по `start_at` ↑, группировка по дням со sticky-заголовками; заезды с `free_seats = 0` — бейдж «Мест нет», некликабельны.
- Состояния списка: [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md) — Loading/Content/Empty/Error, pull-to-refresh.

**Условия доступности:**
- Карточка заезда кликабельна (открывает SCR-003), если: `free_seats > 0`.
