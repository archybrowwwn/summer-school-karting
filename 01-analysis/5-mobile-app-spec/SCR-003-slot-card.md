...
| `configuration.name` | string | Название конфигурации трассы (заголовок блока «Конфигурация трассы»). |
| `configuration.type` | enum `novice` / `experienced` | Бейдж типа (новичковая / опытная) с текстовой подписью. |
...
| `marshal.name` | string | Имя маршала-инструктора (без контактов). |
...
| `free_rental_equipment` | integer | Свободно прокатных комплектов экипировки (шлем + подшлемник). |
...
│  ┌─────────────────────────────┐ │
│  │ Конфигурация трассы         │ │
│  │ Короткая трасса             │ │  ← configuration.name
│  │ [ Новичковая ] · ~20 мин    │ │  ← configuration.type · configuration.duration_min (если есть)
│  └─────────────────────────────┘ │
│  ┌─────────────────────────────┐ │
...
│  ┌─────────────────────────────┐ │
│  │ Маршал-инструктор           │ │
│  │ Алексей                     │ │  ← marshal.name (без контактов)
│  └─────────────────────────────┘ │
│  ┌─────────────────────────────┐ │
│  │ Места (карты)               │ │
...
│  │ Свободно 7 комплектов       │ │  ← free_rental_equipment (числом)
│  └─────────────────────────────┘ │
│  ┌─────────────────────────────┐ │
│  │ Цена                        │ │
...
| Блок «Конфигурация трассы» | `configuration.name` + бейдж `configuration.type` + длительность `configuration.duration_min` | Да (длительность — опционально) |
| Блок «Схема трассы + место сбора» | Статичный превью + пин + текст `meeting_point` | Да |
| Блок «Маршал-инструктор» | `marshal.name` | Да |
...
| Заголовок | «Карточка заезда» / `configuration.name` | `configuration.name` из `getSlot` | — | — |
...
| Описание заезда | «Заезд на конфигурации "`configuration.name`" займёт `configuration.duration_min` мин и подойдёт для `{аудитория по configuration.type}`» | `Configuration.description` из `getSlot` **при наличии**; иначе составляется на клиенте из `configuration.name` + `configuration.duration_min` + `configuration.type` | — | — |
| Дата и время | Старт заезда, крупно | `start_at` из `getSlot` | — | — |
| Название конфигурации | Заголовок блока «Конфигурация трассы» | `configuration.name` из `getSlot` | — | — |
| Бейдж типа конфигурации | «Новичковая» / «Опытная» (текст, не только цвет) | `configuration.type` из `getSlot` | — | — |
...
| Маршал-инструктор | Имя, без контактов | `marshal.name` из `getSlot` | — | — |
| Места (карты) | «Свободно `free_seats` из `total_seats`» | `free_seats`, `total_seats` из `getSlot` | — | — |
| Прокатная экипировка | «Свободно `free_rental_equipment` комплектов» | `free_rental_equipment` из `getSlot` | — | — |
...
- Описание заезда: приоритетно берётся серверное `Configuration.description`; при его отсутствии собирается на клиенте из `configuration.name` + `configuration.duration_min` + аудитории по `configuration.type` («новичковая» → «подойдёт для новичков», «опытная» → «для опытных»). Числа (минуты) — из данных, **не хардкод**; сегмент длительности скрывается, если `configuration.duration_min` отсутствует.
...
| ID | Название | Приоритет |
|----|----------|-----------|
| — | Модель `Slot` — read-only (поля configuration.*, marshal.*, free_rental_equipment и т.д.) | Must |
