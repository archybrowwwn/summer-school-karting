...
**Зона авторизации:** НЗ  
**Дизайн-макет:** Figma — [Шаг 1 ввод номера](https://www.figma.com) · [вход по номеру](https://www.figma.com) · [Шаг 2 ввод кода](https://www.figma.com) · [Шаг 3 ввод имени](https://www.figma.com)  
(макеты адаптированы под картинг-центр «Апекс»; конкретные node-id заменены на плейсхолдеры до финализации дизайна)

> **OTP — segmented-поле** ([RR-Dxx](../3-design-brief/design-review.md)): поле кода (шаг 2)
...
|-------|-----|-------------------|
| 0.1.0 | SCR-001 «Регистрация / Вход» | Первоначальная версия ТЗ: трёхшаговый OTP-вход без пароля (телефон → код → имя для нового). Адаптировано под картинг-центр «Апекс» на основе брифа заказчика. |
...
- Сохранённая сессия пропускает экран входа при последующих запусках — пользователь сразу попадает в список заездов.
...
|------------|---------|------------------------|
| [SCR-002 Список заездов](SCR-002-slot-list.md) | Подтверждение кода известного номера (`verifyAuthCode` 200, `is_new = false`) | — (сессия установлена) |
| [SCR-002 Список заездов](SCR-002-slot-list.md) | Завершение регистрации нового номера (`updateProfile` 200 после шага 3) | — (сессия установлена) |
...
|--------|-----------------|----------|
| [LOGIC-001 OTP-авторизация](09_Логики/LOGIC-001_OTP_Authorization.md) | Проверка сессии при старте; кнопки «Получить код» (шаг 1), «Подтвердить» (шаг 2), «Продолжить» (шаг 3) | Трёхшаговый вход без пароля по OTP: телефон → код → имя (для нового); выдача и хранение пары токенов (`access_token` + `refresh_token`), refresh-on-401 (POST `/auth/refresh`, ротация), ветвление по `is_new`, обработка таймера/ошибок. |
...
> Все API-запросы экрана — REST. Базовый URL — из конфигурации (prod `https://api.apexkarting.example/v1`).
...
**Логика:**
- Кнопка «Получить код»: [LOGIC-001 OTP-авторизация](09_Логики/LOGIC-001_OTP_Authorization.md) — при тапе валидируется формат телефона, отправляется `requestAuthCode`; из ответа сохраняются `ttl_seconds`/`resend_after_seconds`, выполняется переход на шаг 2 с запуском таймера повтора.
