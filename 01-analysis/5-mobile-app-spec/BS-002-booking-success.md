...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Запись подтверждена (71:6161)](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6161)
...
на бэкенде (`createBooking` → 201), карты уменьшены. Все ошибки записи (нехватка картов, нехватка
...
разрешения на push-уведомления ([LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md)).
...
> сводку брони (когда, какая конфигурация трассы, маршал, сколько картов и экипировки, сколько платить и как),
...
> [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md)).
...
| `is_first_booking` | Параметр навигации (поле ответа `createBooking`) | `true` / `false` | Признак того, что эта бронь — первая успешная запись клиента. **Источник — поле ответа `createBooking`** (`CreateBookingResponse.is_first_booking`, R-006), передаётся в навигацию **вместе с `booking`** при открытии экрана. Экран собственных запросов для его вычисления не делает. Управляет показом системного запроса push-разрешения через [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md). |
| `reminder_hours` | Параметр навигации (поле ответа `createBooking`) | массив целых, напр. `[24, 2]` / пусто | Часы повторных напоминаний из `CreateBookingResponse.reminder_hours` (R-006). Передаётся вместе с `booking`; используется в подводке [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md) («Напомним за N часов до старта»). |
| `push_permission_requested` | Локальный кэш | `true` / `false` (по умолчанию `false`) | Флаг «системный запрос разрешения на push уже показывался на устройстве». См. [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md). |

> Все значения сводки (время старта, конфигурация трассы, маршал, число картов, разбивка экипировки, итоговая
...
запускается [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md) (не блокирующий UI; влияет
...
> Экран не делает вызовов REST API. Системный запрос разрешения на push ([LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md))
> — это обращение к системному API ОС, а не к REST API «Апекс Карт».
...
| `slot.route.name` | string | Название конфигурации трассы |
| `slot.instructor.name` | string | Имя маршала |
| `seats_count` | integer `1…8` (или по capacity слота) | Число забронированных картов (участников) |
| `rental_count` | integer `0…seats_count` | Число прокатных комплектов экипировки; своих = `seats_count − rental_count` |
...
| `slot.price` | integer (RUB) | Тариф за один карт (в `booking.slot`, R-005); для разбивки |
| `slot.rental_price` | integer (RUB) | Тариф проката экипировки за один комплект (в `booking.slot`, R-005); для разбивки |
...
│  │ 〰  Конфигурация: <slot.route.name>   │ │  ← сводка брони
...
> Полноэкранный успех (макет [71:6161](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6161)):
...
> ([LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md)) с подводкой «Напомним за N часов до
> старта», где часы = `reminder_hours` из ответа `createBooking` (не хардкод). Иконки в вайрфрейме
> условны; ключевые числа (старт, картов, цена) — крупные и контрастные. Плейсхолдер `<price_total>` —
...
| Сводка брони | Карточка «лейбл → значение»: старт, конфигурация трассы, маршал, число картов, разбивка экипировки, итог цены. | Да |
...
| Конфигурация трассы | «Конфигурация: …» | `booking.slot.route.name` | — | — |
| Маршал | «Маршал: …» | `booking.slot.instructor.name` | — | — |
| Число картов | «Картов: N» | `booking.seats_count` | — | — |
...
- После отрисовки сводки (до взаимодействия с кнопками) выполняется
  [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md): при первой брони — подводка и
...
| Content — запрос push поверх (при первой брони) | `is_first_booking = true` И `push_permission_requested = false` (после показа сводки) | Поверх сводки — подводка и системный диалог push ([LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md)). **Не отдельное состояние экрана:** UI BS-002 не меняется, кнопки остаются активны. |
...
| Решение по push (при первой брони) | Системный диалог push | Allow / Deny | [LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md): allow → регистрация push-токена; deny → ничего не блокируется, запрос не повторяется |
...
| US-5 | Записаться на заезд и получить подтверждение записи |
...
| LOGIC-007 | [Запрос push-разрешения](09_Логики/LOGIC-007_Push_Permission_Request.md) | — |
...
|----|----------|-----------|
| AC-001 | **Дано** клиент подтвердил запись на SCR-004 и бронь успешно создана (201), **Когда** открывается BS-002, **Тогда** виден блок успеха (иконка + «Вы записаны») и сводка брони: дата/время старта, конфигурация трассы, маршал, число картов, разбивка экипировки и итог цены — все значения из объекта `booking`, без хардкода. | P0 |
...
| AC-003 | **Дано** открыт BS-002, **Когда** клиент нажимает «Мои бронирования», **Тогда** выполняется переход на [SCR-005](SCR-005-my-bookings.md), и созданная бронь присутствует в списке. | P0 |
| AC-004 | **Дано** в брони есть прокатная экипировка (`rental_count > 0`), **Когда** открыта BS-002, **Тогда** сводка показывает соотношение своей (`seats_count − rental_count`) и прокатной (`rental_count`) экипировки. | P1 |
| AC-005 | **Дано** это первая успешная запись клиента (`is_first_booking = true` из ответа `createBooking`, `push_permission_requested = false`), **Когда** показана сводка на BS-002, **Тогда** после сводки показывается подводка «Напомним за N часов до старта» (часы из `reminder_hours` ответа `createBooking`, не хардкод) и вызывается системный запрос разрешения на push ([LOGIC-007](09_Логики/LOGIC-007_Push_Permission_Request.md)). | P1 |
...
| AC-E03 | **Дано** максимум картов (`seats_count = 8`) и длинная сводка, **Когда** контент не помещается, **Тогда** содержимое скроллится внутри экрана, а кнопки «Мои бронирования» и «Готово» остаются достижимыми. | P2 |
