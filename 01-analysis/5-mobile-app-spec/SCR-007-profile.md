...
**Зона авторизации:** АЗ  
**Дизайн-макет:** [Figma — Профиль (71:6677)](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6677) · [Изменение профиля (71:6690)](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6690)
...
| 0.3.0 | SCR-007 «Профиль клиента» | Адаптировано под картинг-центр «Апекс» на основе brief-karting.md: изменена доменная терминология (заезды вместо заездов, маршал вместо инструктора и т.д.), обновлены User Story, бизнес-ценность, последствия удаления аккаунта, названия табов и примеры в критериях приёмки. Стиль и структура сохранены точно как в оригинале. |
...
- **Удаление аккаунта** (деструктивно, ПДн) → `deleteAccount` (DELETE `/profile`) с обязательным явным подтверждением → [SCR-001](SCR-001-registration.md). **Последствия удаления (R-009):** активные брони (`status = active`) **аннулируются и освобождают места и прокатную экипировку** в своих заездах; прошедшие брони **анонимизируются** (обезличиваются от ПДн), но остаются в данных центра для статистики.
...
> Как клиент картинг-центра «Апекс», я хочу видеть и при необходимости менять своё имя и номер телефона, а также безопасно выходить из аккаунта,
> чтобы быть уверенным, что вошёл под своими данными, и чтобы под моим аккаунтом не записались другие на чужом устройстве на заезд.
...
|--------|-----------------|----------|
| [LOGIC-001 OTP-авторизация](09_Логики/LOGIC-001_OTP_Authorization.md) | Смена телефона: подтверждение инлайн-правки поля «Телефон» → шаг кода | Подтверждение нового номера кодом из SMS (как шаг 2 SCR-001): `requestPhoneChangeCode` → ввод кода (segmented, RR-D05) → `confirmPhoneChange`; таймер повтора, обработка ошибок кода. |
...
| [LOGIC-007 Запрос push-разрешения](09_Логики/LOGIC-007_Push_Permission_Request.md) | — | **Не применяется.** Запрос разрешения на push показывается только после первой успешной записи на [BS-002](BS-002-booking-success.md); на экране профиля он **не запрашивается** (управления уведомлениями в MVP нет — см. [foundations §8.1](../3-design-brief/00-foundations.md#81-напоминания-и-уведомления-fr-33-nfr-13)). |
...
### Просмотр + инлайн-карандаши (макет [71:6677](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6677))
...
### Инлайн-правка одного поля (макет [71:6690](https://www.figma.com/design/kART123ptRqmhdWyDlTpDM5/Апекс-картинг?node-id=71-6690))
...
- Каждое поле правится и сохраняется **независимо** (RR-D04):
  ...
  - **Телефон:** при тапе «✓» валидация → [LOGIC-001 OTP-авторизация](09_Логики/LOGIC-001_OTP_Authorization.md): [requestPhoneChangeCode](#requestphonechangecode) → шаг кода (segmented, RR-D05) → [confirmPhoneChange](#confirmphonechange).
...
**Логика:**
- Кнопка «Подтвердить»: [LOGIC-001 OTP-авторизация](09_Логики/LOGIC-001_OTP_Authorization.md) — отправка `confirmPhoneChange`; при 200 телефон обновлён, возврат в просмотр.
...
| AC-006 | **Дано** клиент на экране «Профиль», **Когда** он нажимает «Удалить аккаунт» и подтверждает деструктивное действие, **Тогда** отправляется `deleteAccount`, аккаунт и ПДн удаляются, **активные брони аннулируются и освобождают места и прокатную экипировку в заездах**, прошедшие **анонимизируются** (R-009), сессия завершается и он попадает на [SCR-001](SCR-001-registration.md). | P0 |
