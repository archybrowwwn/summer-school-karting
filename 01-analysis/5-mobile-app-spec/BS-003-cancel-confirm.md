...
- При ранней отмене (≥ 10 мин до старта) карт возвращается в заезд и становится доступен другим клиентам (FR-17).
- При поздней отмене (< 10 мин до старта) запись честно фиксируется без освобождения карта и без денежных штрафов — снимает тревогу клиента (FR-18, принцип P6).
...
|--------|-----------------|----------|
| [LOGIC-004 Отмена правило 10 минут](09_Логики/LOGIC-004_Отмена-правило-10-минут.md) | Инициализация шторки + кнопка «Подтвердить отмену» | Определение типа отмены по `slot.start_at − now`: `≥ 10 мин` (включая ровно 10 мин) → ранняя (карт освобождается); `< 10 мин` → поздняя (карт не освобождается, штрафов нет). Подбор вариации контента и обработка результата `cancelBooking`. |
...
| **Успех 200** | `status = late_cancel` (поздняя отмена, < 10 мин) — **успешный исход, НЕ ошибка** | Закрыть шторку → возврат на SCR-006 с обновлённым статусом «Поздняя отмена»; карт не освобождён, штраф не взимается. Снек-итог «Поздняя отмена: карт не освобождён (правило 10 минут). Штраф не взимается.» показывает **экран-родитель SCR-006** после закрытия шторки ([00-foundations §6.1/§6.2](../3-design-brief/00-foundations.md)). Это **итог поздней отмены, а не ошибка** — шторка ошибочный снек не показывает |
...
> **Итоговый статус определяет сервер (источник истины), сравнение — в UTC** (как в [LOGIC-004](09_Логики/LOGIC-004_Отмена-правило-10-минут.md)). Граница ровно 10 мин → ранняя. Клиентский предрасчёт влияет только на контент шторки до подтверждения.
...
> - `late_cancel` (**HTTP 200**) — **успешный исход** поздней отмены (< 10 мин): приходит как **успех, а не ошибка**, итог-снек «Поздняя отмена: карт не освобождён (правило 10 минут). Штраф не взимается.» показывает экран-родитель SCR-006 ([§6.2](../3-design-brief/00-foundations.md)).
>
...
│                                       │
│  Отмена не позднее чем за 10 минут до │  ← правило 10 минут
...
| Правило 10 минут | Единая формулировка политики отмены из foundations §6 | Да |
| Блок текущего случая | Последствия для этой брони (ранняя / поздняя), вариативный | Да |
| Бейдж «Поздняя отмена» | Пометка (текст + форма/иконка), только в вариации «поздняя» | Опционально (только при `< 10 мин`) |
...
| Правило 10 минут | «Отмена не позднее чем за 10 минут до старта — карт освобождается. Позже — карт остаётся за вами, но штрафов нет.» | Константа (foundations §6) | — | — |
| Блок «ранняя отмена» | «Карты возвращаются в заезд и станут доступны другим.» | Предрасчёт (`slot.start_at − now ≥ 10 мин`) | — | — |
| Блок «поздняя отмена» | «Поздняя отмена: карт не освобождён (правило 10 минут). Штраф не взимается.» + бейдж «Поздняя отмена» | Предрасчёт (`slot.start_at − now < 10 мин`) | — | — |

**Логика:**
- Блок текущего случая: [LOGIC-004 Отмена правило 10 минут](09_Логики/LOGIC-004_Отмена-правило-10-минут.md) — выбор вариации по предрасчёту `slot.start_at − now` относительно порога 10 минут (ровно 10 мин → ранняя).
...
|-----------|---------|-------------|
| Content — ранняя отмена | `slot.start_at − now ≥ 10 мин` (включая ровно 10 мин) | Заголовок + правило 10 минут + блок «карты возвращаются в заезд»; кнопки активны |
| Content — поздняя отмена | `slot.start_at − now < 10 мин` | Заголовок + правило 10 минут + блок «карт не освобождён, штраф не взимается» + бейдж «Поздняя отмена»; кнопки активны |
...
| FR-17 | Ранняя отмена (≥ 10 мин): освобождение карта обратно в заезд | Must |
| FR-18 | Поздняя отмена (< 10 мин): фиксация статуса «поздняя отмена» без освобождения карта и без штрафов | Must |
...
|----|----------|-----------|
| AC-001 | **Дано** до старта осталось 10 минут или больше (`slot.start_at − now ≥ 10 мин`), **Когда** клиент открывает BS-003, **Тогда** показана вариация ранней отмены с правилом 10 минут и сообщением «Карты возвращаются в заезд и станут доступны другим». | P0 |
...
| AC-003 | **Дано** до старта осталось меньше 10 минут (`< 10 мин`), **Когда** клиент открывает BS-003, **Тогда** показана вариация поздней отмены с бейджем «Поздняя отмена», сообщением «карт не освобождён (правило 10 минут)» и явным указанием «Штраф не взимается». | P0 |
...
| AC-N05 | **Дано** клиент подтвердил отмену до старта < 10 мин, **Когда** `cancelBooking` возвращает **200 `late_cancel`**, **Тогда** это трактуется как **успешный исход** (не ошибка): шторка закрывается, а итог-снек «Поздняя отмена: карт не освобождён (правило 10 минут). Штраф не взимается.» показывает экран-родитель SCR-006 ([§6.2](../3-design-brief/00-foundations.md)). | P1 |

### Граничные условия (Edge Cases)

| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** до старта осталось **ровно 10 минут** (`slot.start_at − now = 10 мин`), **Когда** клиент открывает BS-003, **Тогда** показана вариация **ранней** отмены (граница `≥ 10 мин` трактуется как ранняя → карт освобождается). | P0 |
